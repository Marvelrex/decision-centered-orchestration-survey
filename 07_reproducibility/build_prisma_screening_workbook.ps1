...
try {
    ...
    $readmeSubtitle = if ($isAutoScreened) { "Deterministic title and abstract keyword screening for the five database exports searched on 2026-08-28" } else { "Reconstruction for the five database exports searched on 2026-08-28" }
    ...
    $summaryRows = @(
        ...
        @("Current corpus matches in ledger", ('=COUNTIF(''Screening Ledger''!$D$2:$D$' + $ledgerEndRow + ',"<>")'), "Protected matches to reports already held in the corpus")
    )
